...
first when i run the command
        - 10.244.0.16
          10.244.0.17
          10.244.0.15

Verify: Are all 3 pods running? Note down their IP addresses. - yes all are running and note down their IP addresses
...
Verify: What does the EXTERNAL-IP column show? Why is it <pending> on a local cluster?
        ...
        - because there is no cloud controller i s available to provision an external load balancer
